...
export const createUser = async (req, res) => {
  ...
  const { usuario, contrasenia, nombre_para_mostrar, avatar_id, correo, rol } =
    req.body; // Obtener los datos del cuerpo de la petición
...
export const updateUser = async (req, res) => {
  ...
  try {
    const { nombre_para_mostrar, avatar_id, correo, rol } = req.body; // Campos a actualizar

    // Crear un objeto con los campos a actualizar
    const updateData = {};

    if (nombre_para_mostrar !== undefined) {
      // Si se proporciona el campo `nombre_para_mostrar`
      updateData.nombre_para_mostrar = nombre_para_mostrar;
    }

    if (avatar_id !== undefined) {
      // Si se proporciona el campo `avatar_id`
      updateData.avatar_id = avatar_id;
    }

    if (correo !== undefined) {
      // Si se proporciona el campo `correo`
      updateData.correo = correo;
    }

    if (rol !== undefined) {
      // Si se proporciona el campo `rol`
      updateData.rol = rol;
    }
